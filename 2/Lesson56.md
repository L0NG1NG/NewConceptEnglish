# Faster than sound

> 复习形容词/副词的比较级/最高级
其他表示比较的词/词组

## 译文

1. 旧式汽车的比赛每年举行一次。
2. 去年有很多汽车参加了这项比赛。比赛开始之前，人们异常激动。
3. 最漂亮的汽车之一是罗尔斯--罗伊斯生产的银鬼汽车。
4. 而最不寻常的一辆则要属只有3只轮子的奔驰牌汽车了。
5. 该车造于1885年，是参赛车中最老的一辆。
6. 在好一阵喧闹的爆炸声之后，比赛开始了。
7. 很多汽车在途中就抛了锚，而有些驾驶员花在汽车底下的时间比坐在汽车里面的时间还长。
8. 然而还是有几辆汽车跑完了全程。获胜的那辆车达到了时速40英里--远远超过任何对手。
9. 它在接近终点时，冲下了山坡，驾驶员费了好大劲才把车停下来。
10. 这次比赛使每个人都挺开心。
11. 它虽然与现代汽车比赛大不相同，但激动人心的程度并不亚于现代化汽车大赛。

## 原文

1. Once a year, a race is held for old cars.[^1]
2. **A lot of** cars entered for this race last year and there was **a great deal of** excitement just before it began.[^2]
3. One of **the most handsome** cars was a Rolls-Royce Silver Ghost.
4. **The most unusual** car was a Benz which had only three wheels.
5. **Built** in 1885, it was the oldest car **taking** part.[^3]
6. After **a great many** loud explosions, the race began.
7. Many of the cars broke down on the course and some drivers spent **more time** under their cars **than** in them!
8. A few cars, however, completed the race. The winning car reached a speed of forty miles an hour -- **much faster than** any of its rivals.
9. It sped downhill at the end of the race and its driver had a lot of trouble trying to stop it.
10. The race gave everyone **a great deal of** pleasure.
11. It **was** very **different from** modern car races but **no less exciting**.[^4]

## 单词

| 单词 | 含义 |
| --- | --- |
| explosion | n.爆炸，轰响 |
| course | n.跑道，行程；课程，一道菜 |

## 许多、大量

修饰或指代**不可数**名词：`much`、`a great deal of`、`a lot of`  
修饰或指代**可数**名词：`many`、`a great many of`、`a great number of`、`a lot of`

## 少、没那么多

修饰或指代**不可数**名词：

1. `a little`: 肯定的表达，**还有一点**
    - There isn't much whisky,but you can have a little.
2. `little`: 否定消极的表达，表示**没多少**了
    - There is little I can do to help him.

修饰或指代**可数**名词：

1. `a few`:有几个，肯定的表达
2. `few`:没剩几个，否定的表达

## 易混淆词

| 单词 | 侧重点 | 常用对象 | 形象参考 |
| --- | --- | --- | --- |
| Handsome | 气质、英俊、匀称 | 男性（偶尔指大气女性） | 西装革履的绅士 / 英气的女军官 |
| Beautiful | 极致美、生命力、神韵 | 女性、风景、艺术 | 盛装的女神 / 壮丽的日落 |
| Pretty | 甜美、娇小、可爱 | 女孩、年轻女性、花朵 | 邻家女孩 / 精致的小饰品 |
| Good-looking | 颜值高、外貌出众 | 所有人 | 街上回头率高的帅哥美女 |

**reach & arrive in/at**  

这两个词都表示“到达”，但在语法结构和语气上有一些关键区别。  
最简单的记法是：去大地方用 arrive in，去小地方用 arrive at；如果你想省掉介词或者强调好不容易才走到，就用 reach。

**take part & take place**  

- It was the oldest car **taking part**.(表示参加、参与)
- The next race will **take place** in a year's time.（表示发生、举行）

[^1]: `race`专门指比速度的比赛，而`competition`可以表示各种各样的比赛。
[^2]: `a lot of` 既可以接可数名词复数也能接不可数名词,`a great deal of`只能接不可数名词。
[^3]: 两个非谓语动词分别表示**被建造**和**主动参加**。
[^4]:`but`前后两句共享的是`it was`，所以后面的被省略掉。
